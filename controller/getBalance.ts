...
try {
    ...
    const response: AxiosResponse = await axios.get('https://topups-sandbox.reloadly.com/accounts/balance',
    {
        headers: {
    ...
    if (axios.isAxiosError(error)){
        ...
        if (axiosError.response){
            ...
            res.status(axiosError.response.status).json({axiosError: 'Server Error in generating Airtime'})
